...
module.exports = {
    bookSalon: async (req, res) => {
        try {
            if (!req.body.salon) return res.status(400).json({ message: 'salon - id field is required' })
    ...
    verifyPayment: async (req, res) => {
        try {
            ...
            if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature || !salonId || !slotTime || !slotDate || !price) return res.status(400).json({ messaage: 'razorpay_order_id, razorpay_payment_id, razorpay_signature, salonId, slotTime, slotDate, price - fields required' })
            ...
            if (razorpay_signature === expectedSign) {
                await bookings.create({ orderId: razorpay_order_id, userId: req._id, salonId, slotTime, slotDate, price: setPrice })
    ...
    getBookings: async (req, res) => {
        try {
            bookings.find({ userId: req._id }).populate('salonId').then(response => {
