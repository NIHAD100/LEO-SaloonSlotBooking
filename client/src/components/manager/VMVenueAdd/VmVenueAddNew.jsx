...
function VmVenueAddNew() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!venueName || !mobile || !district || !place || !actualPrice || !discountPercentage || !description || !image || !document || !slotsCount || !lat || !lng) return;
    ...
    try {
      let response = await axios.post(
        "/vm/salon",
        { venueName, mobile, district, place, actualPrice, discountPercentage, description, image: imageUrl, document: documentUrl, slots, lat, lng },
        {
          headers: {
            Authorization: token,
          },
  ...
  return (
    <div className="p-4 sm:ml-64">
      <form className="p-4 mt-14" onSubmit={(e) => handleSubmit(e)}>
        ...
        <div className="pb-6">
          <button className="bg-green-500 text-lg hover:bg-green-600 duration-300 text-white px-2 py-1 flex justify-center float-right w-28 rounded shadow-md " disabled={submitLoad}>
            {submitLoad ? (
